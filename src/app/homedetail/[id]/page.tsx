...
export default function HomeDetailPage({ params }: PageProps) {
  ...
  const [maintenanceFormState, setMaintenanceFormState] = useState<MaintenanceFormEntry[]>([])
  ...
  const [editingPartDraft, setEditingPartDraft] = useState<PartFormEntry | null>(null)
  ...
  const buildMaintenanceFormState = (targetAsset: Vehicle | null): MaintenanceFormEntry[] => {
    if (!targetAsset?.maintenance) return []

    return targetAsset.maintenance.map((item) => {
      const dueDate =
        item.maintenanceEndDate && !Number.isNaN(new Date(item.maintenanceEndDate).getTime())
  ...
  const handleMaintenanceSave = async (event: FormEvent<HTMLFormElement>) => {
    ...
    for (const entry of maintenanceFormState) {
      ...
      if (dateValue) {
        ...
        if (Number.isNaN(parsedDate.getTime())) {
          setMaintenanceError('Please provide valid dates for maintenance items.')
          return
        }
    ...
    try {
      ...
      setMaintenanceError('Unable to save maintenance changes. Please try again.')
    } finally {
  ...
  const handlePartDraftChange = useCallback(
    (field: keyof PartFormEntry, value: string) => {
      setEditingPartDraft((prev) => (prev ? { ...prev, [field]: value } : prev))
    },
    []
  )
    ...
    return [
        ...
        cell: ({ row }) => {
          ...
          return isEditing ? (
            <input
              ...
              onChange={(event) => handlePartDraftChange('part', event.target.value)}
        ...
        cell: ({ row }) => {
          ...
          return isEditing ? (
            <input
              ...
              onChange={(event) => handlePartDraftChange('type', event.target.value)}
              className="w-full rounded-md border border-gray-300 px-2 py-1 text-sm"
            />
          ) : (
            row.original.type || '—'
          ...
          if (isEditing) {
            return (
              <input
                ...
                onChange={(event) => handlePartDraftChange('date', event.target.value)}
                className="w-full rounded-md border border-gray-300 px-2 py-1 text-sm"
              />
            )
          }
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          if (isEditing) {
            return (
              <input
                ...
                onChange={(event) => handlePartDraftChange('url', event.target.value)}
                className="w-full rounded-md border border-gray-300 px-2 py-1 text-sm"
                placeholder="https://"
              />
            )
          ...
          const href =
            url ||
            (part ? `https://www.amazon.com/s?k=${encodeURIComponent(part)}` : '')
          ...
          {asset ? (
            <div className="space-y-6">
              ...
              <section className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                  ...
                  {isEditingQuickInfo ? (
                    <form onSubmit={handleQuickInfoSave} className="mt-4 space-y-4">
                      {quickInfoError && (
                      ...
                      <div className="flex justify-between">
                        ...
                        <dd>
                          {asset.odometer && asset.odometer.length > 0
                            ? `${asset.odometer[asset.odometer.length - 1]?.odometer ?? 'N/A'}`
                            : 'N/A'}
                        </dd>
                      </div>
                      <div className="flex justify-between">
                        ...
                        <dd>
                          {asset.oilChange && asset.oilChange.length > 0
                            ? new Date(
                                asset.oilChange[asset.oilChange.length - 1]?.date ??
                                  Date.now()
                              ).toLocaleDateString()
                            : 'N/A'}
                        </dd>
              ...
              <section className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          if (maintenanceFormState.length === 0) {
                            setMaintenanceFormState([{ maintenanceType: '', maintenanceDesc: '', maintenanceEndDate: '' }])
                          }
                        }}
                        className="rounded-md border border-green-400 bg-green-100 px-3 py-1 text-sm font-medium text-green-700 transition hover:bg-green-200"
                      >
                  ...
                  {isEditingMaintenance ? (
                    <form onSubmit={handleMaintenanceSave} className="mt-4 space-y-4">
                      {maintenanceError && (
                        ...
                        maintenanceFormState.map((entry, index) => (
                          ...
                          >
                            ...
                            <div className="flex justify-end">
                              <button
                                type="button"
                                onClick={() => handleRemoveMaintenanceEntry(index)}
                        ...
                        return (
                          ...
                          >
                            ...
                            {item.maintenanceDesc && (
                              <p className="mt-1 text-gray-600">{item.maintenanceDesc}</p>
                            )}
              ...
              <section className="rounded-lg border border-gray-200 bg-white p-6 shadow-sm">
                  <div className="flex items-center justify-between">
                    <h2 className="text-xl font-semibold text-gray-900">Parts to Order</h2>
                    <button
                      type="button"
                      onClick={handleAddPartEntry}
                      disabled={isSavingParts || isUpdatingPart || editingPartRowId !== null}
                  ...
                  {!isEditingParts && updatePartError && (
                    <p className="mt-2 text-sm text-red-600">{updatePartError}</p>
                  )}
                  {isEditingParts ? (
                    <form onSubmit={handlePartsSave} className="mt-4 space-y-4">
                      {partsError && <p className="text-sm text-red-600">{partsError}</p>}
                          ...
                          >
                            <div>
                              ...
                              <input
                                ...
                                value={entry.part}
                                onChange={(event) =>
                                  handlePartFieldChange(index, 'part', event.target.value)
                                }
                                className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
                              />
                            </div>
                            <div>
                              ...
                              <input
                                ...
                                value={entry.type}
                                onChange={(event) =>
                                  handlePartFieldChange(index, 'type', event.target.value)
                                }
                                className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
                              />
                            </div>
                            <div>
                              ...
                              <input
                                ...
                                value={entry.url}
                                onChange={(event) =>
                                  handlePartFieldChange(index, 'url', event.target.value)
                                }
                                className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
                                placeholder="https://"
                              />
                            ...
                            <div>
                              ...
                              <input
                                ...
                                value={entry.date}
                                onChange={(event) =>
                                  handlePartFieldChange(index, 'date', event.target.value)
                                }
                                className="mt-1 w-full rounded-md border border-gray-300 px-3 py-2 text-sm"
                              />
                            </div>
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-10 text-center shadow-sm">
              ...
              <p className="mt-2 text-sm text-gray-500">
                We couldn&apos;t locate details for this asset. It may have been removed.
              </p>
              <Link
                href="/home"
                className="mt-6 inline-flex items-center rounded-md bg-blue-600 px-4 py-2 text-sm font-medium text-white shadow-sm transition hover:bg-blue-700"
